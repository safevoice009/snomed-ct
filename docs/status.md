...
- **P0 developer-experience upgrades shipped** (inspiration: Deepgram playground / Veryfi demo / Nanonets key-flow — see [`docs/DESIGN_INSPIRATION_DOSSIER.md`](DESIGN_INSPIRATION_DOSSIER.md)): "Copy as cURL" exports the real last parse request; Dev Hub split view mirrors the live API response beside the code snippet; sandbox-key acquisition card; key-dropdown auto-hidden when single key; drag-and-drop prescription upload onto the workbench canvas; no-signup/1,000-free-calls badges. Zero app.js edits — DX layer hooks via guarded fetch interceptor in site.js. Bindings verified, 40/40 tests green.
